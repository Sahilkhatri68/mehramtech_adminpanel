...
function CompletedProductList() {
  ...
  const getCompletedRequestDevice = () => {
    axios
      ...
      .then((res) => {
        console.log(res.data.requestedDevices);
        const formattedCompletedIssue = res.data.requestedDevices.map(
          (device) => {
            const updatedAt = new Date(device.updatedAt);
            const formattedDate = `${updatedAt.getDate()}-${
              updatedAt.getMonth() + 1
            }-${updatedAt.getFullYear()}`;
            const formattedTime = `${updatedAt.getHours()}:${updatedAt.getMinutes()}`;
        ...
        setCompletedIssue(formattedCompletedIssue);
      })
      .catch((err) => {
        console.log(err);
      });
...
export default CompletedProductList;
